...
- 第1次尝试：`mem=4GB`
- 如果失败（内存不足），第2次尝试：`mem=5GB`（4 * 1.25 = 5，向上取整）
- 如果仍然失败，第3次尝试：`mem=7GB`（5 * 1.25 = 6.25，向上取整为 7）

如果同时设置了 `--mem 4` 和 `--h_vmem 5`：

- 第1次尝试：`mem=4GB, h_vmem=5GB`
- 如果失败，第2次尝试：`mem=5GB, h_vmem=7GB`（4*1.25=5, 5*1.25=6.25向上取整为7）
...
1:3    0004   Finished   0      3652315    0        12-09 10:26 
```
